...
copy them into a fresh new directory called `practical7`. The values obtained
below have been obtained with the following specific versions (Feb 9, 2026) of
these files:

  * [mostres_analitzades.csv](mostres_analitzades.csv).
  * [virus_detectats.csv](virus_detectats.csv).
...
code that replaces the questions marks `??????` below.
...
2023 to June 2024:
    ...
    number of rows in the subsetted object `dat2yr2324`.
...
[Factors](https://funcompbio.github.io/lecture6/#26) in R are a class of objects
...
individuals per month in the `data.frame` object `dat2yr2324`. We can use the
...
Here the first argument is the vector (column of `dat2yr2324` in this case)
...
`data.frame` object `dat2yr2324` which column may store the age group). Assuming
